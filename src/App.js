...
import Character from './CharComponent/CharComponent'
...
class App extends Component {
    ...
    removeCharComponentHandler(index) {
        const tmpText = [...this.state.text];
        tmpText.splice(index, 1);
        const updatedText = tmpText.join('');
    ...
    render() {
        return (
            <div className="App">
                ...
                <p>Length of text above is: {this.state.textLength}</p>
                <Validation textLength={this.state.textLength} />
                <p>{this.state.text}</p>
                <div>
                    {this.state.text.split('').map((character,index) => {
                        return <Character key={index} character={character} click={() => this.removeCharComponentHandler(index)}/>
                    })}
                </div>
